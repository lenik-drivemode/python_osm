...
def parse_android_log_speed_data(logd_folder):
    ...
    Returns:
        tuple: (timestamps, speeds_kmh, coordinates)
    """
    try:
        timestamps = []
        speeds_kmh = []
                        ...
                        for nmea_sentence in nmea_matches:
                            try:
                                ...
                                if log_timestamp and current_speed is not None:
                                    ...
                                    # Reset current speed after recording
                                    current_speed = None
    ...
    try:
        ...
        for track in root.iter():
            if track.tag.endswith('Track'):
                ...
                # Look for speed data in ExtendedData
                speed_data = []
                ...
                # Combine timestamps, coordinates, and speeds
...
def filter_data_by_date(timestamps, speeds_kmh, coordinates, filter_date):
    """
    ...
    Returns:
        tuple: Filtered (timestamps, speeds_kmh, coordinates)
    """
    if not timestamps:
        return timestamps, speeds_kmh, coordinates
    ...
    return filtered_timestamps, filtered_speeds, filtered_coords
...
def plot_speed_data(timestamps, speeds_kmh, title="GPS Speed Data", filepath=None):
    """
    Create a plot showing speed over time.
...
def create_detailed_speed_analysis(timestamps, speeds_kmh, filepath=None):
    """
...
Examples:
  ...
  %(prog)s gps_data.txt --format nmea --title "NMEA Speed Analysis"
    ...
    parser.add_argument('--version',
                       action='version',
                       version='GPS Speed Analyzer 1.0.0')
    ...
    if file_type == 'android_logs':
        timestamps, speeds_kmh, coordinates = parse_android_log_speed_data(args.input_path)
    elif file_type == 'nmea':
        timestamps, speeds_kmh, coordinates = parse_nmea_speed_data(args.input_path)
    else:  # kml or fallback
        timestamps, speeds_kmh, coordinates = parse_kml_speed_data(args.input_path)
    ...
    if args.date:
        ...
        timestamps, speeds_kmh, coordinates = filter_data_by_date(
            timestamps, speeds_kmh, coordinates, args.date
        )
